...
There are a few key components of the overall architecture:
  ...
  * `dit-server` - A kotlin web server that provides an API for the dit CLI as well as the 
     docker volume API.
  * `dit` - A Docker image that provides a runtime environment for `dit-server`, as well as
     the means to execute ZFS commands within that container.

## Docker Images

The dit system uses multiple Docker images:

1. **`ditdotdev/dit:latest`** - The main server image containing:
   - Dit server JAR and runtime environment
...
2. **`ditdotdev/zfs-builder:latest`** - ZFS module builder used when:
...
3. **`ditdotdev/ssh-test-server:latest`** - Testing infrastructure:
...
The dit server is built from `server/src`. It is a web server wrapped around a storage
persistence layer, and remote executor framework. The important parts of the server can be
found at:

  * `dev.dit.apis.*` - Entry points for the remote APIs. Very thin layer that translates
    from JSON into native models and then invokes the appropriate backend model.
  * `dev.dit.models.*` - Object models used within the APIs.
  * `dev.dit.metadata.*` - Metadata persistence layer. We run a PostgreSQL database within
    the server container.
  * `dev.dit.context.*` - Provider for context-specific operations, such as managing storage
    and running operations that requires access to storage.
  * `dev.dit.operation.*` - Handlers for asynchronous push and pull operations. The actual
...
manage ZFS within this hidden VM is a non-starter, so we use a shell (`dit-launch`) container that
is responsible for setting up and configuring ZFS, only to then launch the server itself
(`dit-server`), such that the server can access the pool and ZFS commands without having to worry
...
To do this, we have to accomplish a few key things:
  ...
  3. Configure the system such that mounts created within the `dit-server` container will
...
[zfs-releases](https://github.com/ditdotdev/zfs-releases), and fall back to using the
`ditdotdev/zfs-builder:latest` Docker image to build custom modules if no
...
information, see the `bind_mounts()` function in the [dit.sh](server/src/scripts/dit.sh)
utility file.
...
There are three types of tests:
  ...
  * Integration tests - These tests validate the whole dit server program, but still runs within the IDE and
  ...
  * `docker` - Runs local workflows using docker. Should be runnable on any system that supports dit with ZFS.
  * `remote` - Runs tests for each of the remotes. In addition to having dit server running locally with docker,
    these tests require additional configuration, with `S3_LOCATION` set in the environment to a S3 bucket and path
    in the format `bucket/path` (e.g. `dit-maven/titan-e2e-tests`). The bucket must have public read access

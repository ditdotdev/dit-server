...
import dev.dit.metadata.table.Commits
import dev.dit.metadata.table.Operations
import dev.dit.metadata.table.ProgressEntries
import dev.dit.metadata.table.Remotes
import dev.dit.metadata.table.Repositories
import dev.dit.metadata.table.Tags
import dev.dit.metadata.table.VolumeSets
import dev.dit.metadata.table.Volumes
...
/*
 * The metadata provider is responsible for persistence of all metadata to the dit database. With the exception of
 * init(), it's up to the caller to manage transactions.
 */
class MetadataProvider(
    val inMemory: Boolean = true,
    val databaseName: String = "dit",
) {
    companion object {

...
class OpenApiContractTest : StringSpec() {
    ...
    private fun parseOpenApiPaths(): List<Pair<String, String>> {
        val specFile = File("${System.getProperty("user.dir")}/../openapi/dit.yml")
        if (!specFile.exists()) {
            // Try from project root (CI may run from different directory)
            val altFile = File("openapi/dit.yml")
            if (!altFile.exists()) {
    ...
    private fun parseKtorRoutes(): Set<Pair<String, String>> {
        val apiDir = File("${System.getProperty("user.dir")}/src/main/kotlin/dev/dit/apis")
        if (!apiDir.exists()) {
    ...
    init {
        ...
        "every OpenAPI schema has a matching Kotlin model class with required fields non-nullable" {
            ...
            for ((schemaName, schema) in schemas) {
                ...
                if (kotlinClass == null) {
                    problems += "spec schema '$schemaName' has no Kotlin model class at dev.dit.models"
                    continue
                }
                ...
                for (propName in kotlinProps.keys) {
                    if (propName !in specProps) {
                        problems +=
                            ...
                            "(either add it to openapi/dit.yml or remove it from the model)"
                    }
                }
            }
    ...
    private fun parseOpenApiSchemas(): Map<String, SpecSchema> {
        val specFile =
            File("${System.getProperty("user.dir")}/../openapi/dit.yml").takeIf { it.exists() }
                ?: File("openapi/dit.yml").takeIf { it.exists() }
     ...
     * UpperCamelCase under `dev.dit.models`. Spec uses lowerCamelCase
    ...
    private fun resolveModelClass(schemaName: String): KClass<*>? {
        ...
        return try {
            Class.forName("dev.dit.models.$className").kotlin

...
import dev.dit.metadata.OperationData
...
class OperationExecutorTest : StringSpec() {
    ...
    fun getExecutor(data: OperationData): OperationExecutor =
        services.operations.createExecutor(
            data.operation,
            "foo",
            Remote("nop", "origin"),
            RemoteParameters("nop"),
            data.metadataOnly,
        )
        ...
        "sync data for pull succeeds" {
            ...
            verify {
                ...
                context.syncVolumes(
                    any(),
                    any(),
                    listOf(Volume("volume", emptyMap(), mapOf("mountpoint" to "/mountpoint"))),
